#ifndef CONFIGVARIO_H
#define _CONFIGVARIO_H
...
Config_vario(std::string fp_, std::string keydelim,std::string delim) {
	fp = fp_;
			...
			while (getline(cFile, line))
			{
				...
				else
				{
					...
					while ((pos = line.find(delimiter)) != std::string::npos)
					{
						...
						actual_value = Convert::StringToNumber<float>(value);
...
int main(int argc, char* argv[], char* envp[]){
	...
	std::vector<float> nvar,sill;
	std::string delimiter, keydelimiter;
	delimiter = ";";
	keydelimiter = "=";
	Config_vario cfg ("Param.txt" );

	
	nvar= cfg.return_value("nvar");
	auto sills = cfg.return_value("sill");
